...
import com.rene.api.dto.DtoConverter;
...
public class ScheduleQueryService {
	...
	private List<ForListScheduleDto> getSchedulesByPeriod(AuthUser authUser, Period period) {
		return Stream.concat(scheduleRepository.findAllByWriter_Id(authUser.getId())
			...
			engagementRepository.findAllByAttendeeId(authUser.getId())
				...
				.map(engagement -> DtoConverter.toForListDto(engagement.getSchedule()))).collect(toList());
	}

	public List<SharedScheduleDto> getSharedSchedulesByDay(LocalDate date, AuthUser authUser) {
		return Stream.concat(shareService.findSharedUserIdsByUser(authUser).stream(), Stream.of(authUser.getId()))
			.map(userId -> SharedScheduleDto.builder()
				.userId(userId)
				.name(userService.getOrThrowById(userId).getName())
				.me(userId.equals(authUser.getId()))
				.schedules(getSchedulesByDay(date, AuthUser.of(userId)))
				.build())
			.collect(toList());
	}

	public List<SharedScheduleDto> getSharedSchedulesByWeek(LocalDate startOfWeek, AuthUser authUser) {
		return Stream.concat(shareService.findSharedUserIdsByUser(authUser).stream(), Stream.of(authUser.getId()))
			.map(userId -> SharedScheduleDto.builder()
				.userId(userId)
				.name(userService.getOrThrowById(userId).getName())
				.me(userId.equals(authUser.getId()))
				.schedules(getSchedulesByWeek(startOfWeek, AuthUser.of(userId)))
				.build()
	...
	public List<SharedScheduleDto> getSharedSchedulesByMonth(YearMonth yearMonth, AuthUser authUser) {
		return Stream.concat(shareService.findSharedUserIdsByUser(authUser).stream(), Stream.of(authUser.getId()))
			.map(userId -> SharedScheduleDto.builder()
				.userId(userId)
				.name(userService.getOrThrowById(userId).getName())
	...
	public List<SharedScheduleDto> getSharedSchedulesByDay2(LocalDate date, AuthUser authUser) {
		return getSharedSchedulesByFunction(authUser, (Long userId) -> getSchedulesByDay(date, AuthUser.of(userId)));
	}

	public List<SharedScheduleDto> getSharedSchedulesByWeek2(LocalDate startOfWeek, AuthUser authUser) {
		return getSharedSchedulesByFunction(authUser,
			(Long userId) -> getSchedulesByWeek(startOfWeek, AuthUser.of(userId)));
	}

	public List<SharedScheduleDto> getSharedSchedulesByMonth2(YearMonth yearMonth, AuthUser authUser) {
		return getSharedSchedulesByFunction(authUser,
