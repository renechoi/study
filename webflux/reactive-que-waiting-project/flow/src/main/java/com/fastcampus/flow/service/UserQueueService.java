...
public class UserQueueService {
    ...
    public Mono<Long> registerWaitQueue(final String queue, final Long userId) {
        ...
        // filter 에서 false 인 경우 -> Error = 이미 등록된 경우
        // true인 경우 flatmap으로 처리 -> rank -> userqueue에 대해서 숫자값으로 받음 -> rank가 얼마인지를
        // 0번째 대기는 없으므로 1부터 설정해줌
    ...
    public Mono<Long> allowUser(final String queue, final Long count) {
        // 진입을 허용하는 단계
        // 1. wait queue 사용자를 제거 -> popMin -> rank가 가장 작은 것을 꺼냄 (삭제, pop)
        // 2. proceed queue 사용자를 추가 -> value를 동일하게 타임스탬프로 설정
