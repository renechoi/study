package com.example.springbatchprojecthouse.job.notify;
...
import com.example.springbatchprojecthouse.adapter.FakeSendService;
...
import com.example.springbatchprojecthouse.core.repository.AptNotificationRepository;
...
public class AptNotificationJobConfigTest {
	...
	public void success() throws Exception {
		// given -> h2 db에 실제 저장 로직 작성
		LocalDate dealDate = LocalDate.now().minusDays(1);	// 어제자 데이
	...
	private void givenAptDeal(String guLawdCd, LocalDate dealDate) {
		when(aptDealService.findByGuLawdCdAndDealDate(guLawdCd, dealDate)).thenReturn(
			Arrays.asList(new AptDto("IT아파트", 2000000000L), new AptDto("탄천아파트", 1500000000L)));
	}
}
